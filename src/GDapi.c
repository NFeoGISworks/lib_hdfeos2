...
April 26, 2005 Phuong Nguyen Fixed the "prinf" statement for eccentricity
                             projection parameter to have more significant
                             digits
Dec  15, 2005   Tom Roegner  Added defined(CYGWIN) in GDmm2ll_cea
Jun  22, 2007   Abe Taaheri  Added SZIP compression capability.
May  15, 2010   Abe Taaheri  Added Dimension Scale set & get functions
Dec  10  2013   Abe Taaheri  Increased NGRID from 400 to 800 and NGRIDREGN
                             from 512 to 1024. These numbers had been changed
                             in 8/23/2011 from 200 and 256 to 400 and 512 
                             respectively.
******************************************************************************/
#include "stdio.h"
#include "mfhdf.h"
...
intn GDSDfldsrch(int32, int32, char *, int32 *, int32 *, int32 *, int32 *, int32 [], int32 *);
intn GDwrrdfield(int32, char *, char *, int32 [], int32 [], int32 [], VOIDP datbuf);
...
intn GDtangentpnts(int32, float64 [], float64 [], float64 [], float64 [], float64 [], int32 *);
...
intn GDll2mm_cea(int32 projcode,int32 zonecode, int32 spherecode,float64 projparm[], int32 xdimsize, int32 ydimsize, float64 upleftpt[], float64 lowrightpt[], int32 npnts, float64 lon[],float64 lat[], float64 x[],float64 y[], float64 *scaleX,float64 *scaleY);
	 ...
	 float64 upleftpt[], float64 lowrightpt[])
{
    ...
    char            name[UTLSTR_MAX_SIZE];	/* Vgroup name */
    char            class[UTLSTR_MAX_SIZE];	/* Vgroup class */
	...
	HEreport("Cannot call GDcreate on a file opened read-only.\n");
      }

    /* Check gridname for length */
    /* ------------------------- */
    else if ((intn) strlen(gridname) > VGNAMELENMAX)
    {
	status = -1;
	HEpush(DFE_GENAPP, "GDcreate", __FILE__, __LINE__);
...
GDdefcomp(int32 gridID, int32 compcode, intn compparm[])
{
    ...
    uint32          can_encode; /* szip encoder presence flag     */
	...
	status = HCget_config_info(COMP_CODE_SZIP, &can_encode);
#endif
	...
	case HDFE_COMP_DEFLATE:
	    ...
	    /* Set SZIP compression parameter in compression external array */
	case HDFE_COMP_SZIP:
#if defined(H4_HAVE_SZLIB_H) && defined(H4_HAVE_LIBSZ)
	  if(can_encode == (COMP_DECODER_ENABLED|COMP_ENCODER_ENABLED))
	    {
	      if( compparm[0] != 2 && compparm[0] != 4 && 
		  compparm[0] != 6 && compparm[0] != 8 &&
		  compparm[0] != 10 && compparm[0] != 12 && 
		  compparm[0] != 14 && compparm[0] != 16 &&
		  compparm[0] != 18 && compparm[0] != 20 && 
		  compparm[0] != 22 && compparm[0] != 24 &&
		  compparm[0] != 26 && compparm[0] != 28 && 
		  compparm[0] != 30 && compparm[0] != 32)
		{	  
		  status = -1;
		  HEpush(DFE_GENAPP, "GDdefcomp", __FILE__, __LINE__);
		  HEreport("Invalid SZIP compression block size. Valid values are 2 to 32, in multiples of 2.\n");
		  return(status);	       
		}
	      if( compparm[1] != SZ_NN && 
		  compparm[1] != SZ_EC )
		{	  
		  status = -1;
		  HEpush(DFE_GENAPP, "GDdefcomp", __FILE__, __LINE__);
		  HEreport("Invalid SZIP compression type. Valid types are SZ_NN and SZ_EC.\n");
		  return(status);	       
		}
	      GDXGrid[gID].compparm[0] = compparm[0];
	      GDXGrid[gID].compparm[1] = compparm[1];
	    ...
	    {
	      HEpush(DFE_GENAPP, "GDdefcomp", __FILE__, __LINE__);
	      HEreport("Szip does not have encoder; Szip compression won't apply to datafields.\n");
	    }
#endif
	    break;
	}
...
|  Jun 96   Joel Gales    Original Programmer                                 |
|  Jan 97   Joel Gales    Check for metadata error status from EHgetmetavalue |
|  Aug 11   Abe Taaheri   Modified to return default value if it not found in |
|                         structure metadata                                  |
|                                                                             |
|  END_PROLOG                                                                 |
-----------------------------------------------------------------------------*/
...
|  Jan 97   Joel Gales    Check for metadata error status from EHgetmetavalue |
|  Jul 08   Abe Taaheri   Added code to detect compression from SDS if it     |
|                         to find it from structure metadata.                 |
|  Apr 10   AbeTaaheri    Replaced problmatic SDgetcompress with SDgetcompinfo|
|                                                                             |
|  END_PROLOG                                                                 |
-----------------------------------------------------------------------------*/
intn
GDcompinfo(int32 gridID, char *fieldname, int32 * compcode, intn compparm[])
{
    ...
    intn            status = 0;	/* routine return status variable */
    intn            statmeta = 0;	/* EHgetmetavalue return status */
    intn            statusSD;	/* SDgetcompinfo return status */
    int32           fid;	/* HDF-EOS file ID */
    int32           sdInterfaceID;	/* HDF SDS interface ID */
    int32           gdVgrpID;	/* Grid root Vgroup ID */
    int32           idOffset = GDIDOFFSET;	/* Grid ID offset */
    ...
    int32           dims[8];
    char            name[2048];	/* Field Names */
    int32           got_compparm;

    char           *HDFcomp[6] = {"HDFE_COMP_NONE", "HDFE_COMP_RLE",
				  "HDFE_COMP_NBIT", "HDFE_COMP_SKPHUFF",
				  "HDFE_COMP_DEFLATE","HDFE_COMP_SZIP" };	/* Compression Codes */
	...
	if (metaptrs[0] < metaptrs[1] && metaptrs[0] != NULL)
	{
		  ...
		  for (i = 0; i < GDXGrid[gID].nSDS; i++)
		      ...
		      if (GDXGrid[gID].sdsID[i] != 0)
			{
			  ...
			  if(strcmp(name, fieldname) == 0)
			    {
			      statusSD = SDgetcompinfo(sdid, (comp_coder_t *) (compcode), &c_info);
			      /* Note: Per HDF4 users guide this will 
				 return success and compcode as
				 HDFE_COMP_NONE if SDS is not compressed 
				 or no data has been written to SDS */

			      if(statusSD == 0 ) /* Sucess. compcode is 
						    HDFE_COMP_NONE, 
						    HDFE_COMP_NBIT,
						    HDFE_COMP_DEFLATE, 
						    HDFE_COMP_SKIPHUFF, 
						    ...
						    HDFE_COMP_SZIP */
				{
				  ...
				  switch (*compcode)
				    {
				      /* Set NBIT compression parameters 
					 in compression external array */
				    case HDFE_COMP_NBIT:
				      
				      compparm[0] = c_info.nbit.sign_ext;
				      compparm[1] = c_info.nbit.fill_one;
				      compparm[2] = c_info.nbit.start_bit;
				      compparm[3] = c_info.nbit.bit_len;
				      got_compparm = 1;
				      break;
				      
				      /* Set GZIP compression parameter 
					 in compression external array */
				    case HDFE_COMP_DEFLATE:
				      
				      compparm[0] = c_info.deflate.level;
				      got_compparm = 1;
				      break;
				      
				      /* Set SZIP compression parameter 
					 in compression external array */
				    case HDFE_COMP_SZIP:
#if defined(H4_HAVE_SZLIB_H) && defined(H4_HAVE_LIBSZ)
				      compparm[0] = c_info.szip.pixels_per_block;
				      if(c_info.szip.options_mask == 
					 (SZ_EC_OPTION_MASK | SZ_RAW_OPTION_MASK))
					{
					  compparm[1] = SZ_EC;
					  got_compparm = 1;
					}
				      else if(c_info.szip.options_mask == 
					      (SZ_NN_OPTION_MASK | SZ_RAW_OPTION_MASK))
					{
					  compparm[1] = SZ_NN;
					  got_compparm = 1;
					}
					      ...
					      (SZ_RAW_OPTION_MASK))
					{
					  ...
					  HEreport("Could not get valid types SZ_NN and SZ_EC for szip compression.\n");
					  got_compparm = 1;
					}
#endif
				      break;
			      ...
			      else /* SDgetcompinfo failed getting a valid compcode */
				{
				  HEpush(DFE_GENAPP, "GDcompinfo", __FILE__, __LINE__);
				  HEreport(
				  "\"SDgetcompinfo\" failed getting compression info.\n");
				  status = -1;
				  got_compparm = 1;
				}
			      break;
		...
		/*
		 * Get compression parameters if NBIT, DEFLATE 
		 * or SZIP compression
		 */
...
|  May 09   Abe Taaheri   Added code to see if actual dim size is bigger than |
|                         the one in structure metadata                       |
|                                                                             |
|  END_PROLOG                                                                 |
-----------------------------------------------------------------------------*/
intn
...
{
    intn            i;		/* Loop index */
    intn            status,status2;	/* routine return status variable */
    intn            statmeta = 0;	/* EHgetmetavalue return status */
    ...
    *numbertype = -1;
	...
	if (metaptrs[0] < metaptrs[1] && metaptrs[0] != NULL)
	{
		...
		if (strcmp(utlstr, "DFNT_UCHAR8") == 0 || 
		    strcmp(utlstr, "DFNT_UCHAR") == 0 )
		    *numbertype = 3;
		else if (strcmp(utlstr, "DFNT_CHAR8") == 0 || 
			 strcmp(utlstr, "DFNT_CHAR") == 0)
		    *numbertype = 4;
		else if (strcmp(utlstr, "DFNT_FLOAT32") == 0 || 
			 strcmp(utlstr, "DFNT_FLOAT") == 0)
		    *numbertype = 5;
		...
		*rank = ndims;
		...
		if (dims[0] == 0)
		{
		    ...
		    if (status !=0) {
		      HEpush(DFE_GENAPP, "GDfieldinfo", __FILE__, __LINE__);
		      HEreport("GDfieldinfo failed\n", fieldname);
		      return FAIL;
		    }
		}

		/* Some data producesrs may use pure hdf to change 
		   the field dims, which will not change dim values found
		   in structure metadata. Since dims[] found above came 
		   from structure metadata, lets see if actual dims are 
		   larger to replace the output values, to avoid allocating 
		   little memory for the field data later using the values 
		   obtained from this function.
		*/
		
		/* The following correction code written for above problem 
		   caused unexpected errors in grid testdrivers for fillvalue
		   setting because of error in GDfieldinfo. The error comes
		   from GDSDfldsrch for Merged fields. We will ignore dim 
		   recalculation is GDSDfldsrch returns -1 in the 
		   following call. */
		else
		{
		  int32 actualdims[H4_MAX_VAR_DIMS];
		  int32 gID_1;	                        /* GridID - offset */
		  int32 idOffset_1 = GDIDOFFSET;	/* Grid ID offset */
		  int32 solo_1 = 0;	                        /* "Solo" (non-merged) field flag */

		  /* NCR 8050783 fix */
		  /* find out if merged fields or solo. If solo, then set actualdims */

		  /* ------------------------- */
		  status2 = GDSDfldsrch(gridID, sdInterfaceID, fieldname,
				       &sdid, &dum, &dum, &dum, actualdims,
				       &solo_1);

		  if (status2 == 0)
		    {
		      /* If unmerged field then set actualdims */
		      if (solo_1 == 1)
			{
			  for (i = 0; i < ndims; ++i)
			    {
			      if (dims[i] < actualdims[i])
				{
				  dims[i] = actualdims[i];
				}
			    }
...

{
    ...
    int32           temp_compcode = HDFE_COMP_NONE; /* temp Compression code */
    uint32          can_encode;         /* szip encoder presence flag     */

    char           *HDFcomp[6] = {"HDFE_COMP_NONE", "HDFE_COMP_RLE",
				  "HDFE_COMP_NBIT", "HDFE_COMP_SKPHUFF",
				  "HDFE_COMP_DEFLATE","HDFE_COMP_SZIP"};
...
*/
	...
	if (status == 0)
	{
	    ...
	    if(compcode != HDFE_COMP_NONE)
	      {
		temp_compcode = compcode;
	      }
	    else
	      {
		temp_compcode = HDFE_COMP_NONE;
	      }
		...
		else if (compcode == HDFE_COMP_DEFLATE)
		{
		...
		else if (compcode == HDFE_COMP_SZIP )
		  {
		    if(can_encode == (COMP_DECODER_ENABLED|COMP_ENCODER_ENABLED))
		      {
			temp_compcode = compcode;
			/* pixels_per_block is even number from 2-32 */
			c_info.szip.pixels_per_block = GDXGrid[gID].compparm[0];
			if( GDXGrid[gID].compparm[1] == SZ_EC)
			  {
			    c_info.szip.options_mask = SZ_EC_OPTION_MASK | SZ_RAW_OPTION_MASK;
			  }
			else if ( GDXGrid[gID].compparm[1] == SZ_NN)
			  {
			    c_info.szip.options_mask = SZ_NN_OPTION_MASK | SZ_RAW_OPTION_MASK;
			  }
			else
			  {
		    ...
		    else /* encoder is disabled; Would not compress */
		      {
			temp_compcode =  HDFE_COMP_NONE;
		      }
		  }
		...
		/* --------------------------------------------------------- */
		if (temp_compcode != HDFE_COMP_NONE && tilecode == HDFE_NOTILE)
		{
		...
		if (tilecode == HDFE_TILE)
		{
		    ...
		    /* -------------------------- */
		    if (temp_compcode == HDFE_COMP_NONE)
		    {
...
|  BEGIN_PROLOG                                                               |
|                                                                             |
|  FUNCTION: GDdefdimscale                                                    |
|                                                                             |
|  DESCRIPTION: Defines a dimension scale for all fields dimension of a grid  |
|                                                                             |
|                                                                             |
|  Return Value    Type     Units     Description                             |
|  ============   ======  =========   =====================================   |
...
|  dimsize        int32               Dimemsion size value                    |
|  numbertype     int32               dim scale data type such as DFNT_INT32, |
...
|         and sets dimension scale for that field calling GDsetdimscale()     |
...
-----------------------------------------------------------------------------*/
intn
GDdefdimscale(int32 gridID, char *dimname, int32 dimsize, 
	      int32 numbertype, VOIDP data)
{
    ...
    int32           sdInterfaceID;	/* HDF SDS interface ID */
    int32           sdid;	/* SDS object ID */
    int32           gdVgrpID;	/* Swath root Vgroup ID */
    int32           dims[8];	/* Dimension size array */
    int32           rank = 0;	/* Field rank */
    int32           idOffset = GDIDOFFSET;	/* Swath ID offset */

    int32           gID;	/* SwathID - offset */
    int32           rankSDS;	        /* Rank of SDS */
    int32           dum;	        /* Dummy variable */
    char            fieldname[2048];	/* Field Names */
    char            dimlist[HDFE_DIMBUFSIZE];/* Dimension list       */
    int32           Dimindex;
     ...
     * and grid root Vgroup ID
     */
    status = GDchkgdid(gridID, "GDdefdimscale",
		       &fid, &sdInterfaceID, &gdVgrpID);
    ...
    if (status == 0)
    {
      ...
      if ( dimsize == 0 )
	{
	    HEpush(DFE_GENAPP, "GDdefdimscale", __FILE__, __LINE__);
	    HEreport("Invalid (zero) dimension size.\n");
	    status = -1;
	    goto error;
	}
      ...
      if(GDXGrid[gID].nSDS > 0)
	{
	  for (i = 0; i < GDXGrid[gID].nSDS; i++)
	      ...
	      if (GDXGrid[gID].sdsID[i] != 0)
		{
		  ...
		  /* ----------------------------------------- */
		  sdid = GDXGrid[gID].sdsID[i];
		  SDgetinfo(sdid, fieldname, &rankSDS, dims, &dum, &dum);

		  /* if field is MERGED field, continue */
		  if (strstr(fieldname, "MRGFLD_") == &fieldname[0]) continue;

		  status = GDfieldinfo(gridID, fieldname, &rank, dims, &nt, dimlist);
		  if (status != 0)
		    {
		      status = -1;
		      HEpush(DFE_GENAPP, "GDdefdimscale", __FILE__, __LINE__);
		  ...
		  Dimindex = -1;
		  Dimindex = EHstrwithin(dimname, dimlist, ',');
		  if (Dimindex == -1)
		    {
			...
			{
			  foundField = 1;
			  status = GDsetdimscale(gridID, fieldname, dimname, dimsize, 
						 numbertype, NULL);
			}
		      else
			{
		      ...
		      if ( status != 0 )
			{
			  status = -1;
			  HEpush(DFE_GENAPP, "GDdefdimscale", __FILE__, __LINE__);
			  HEreport("Cannot set dimension scale %s for the field %s. \n", 
				   dimname, fieldname);
			  goto error;
			}
		    }
      ...
      if(i == GDXGrid[gID].nSDS && foundField == 0) /* did not find any fields */
	{
	  HEpush(DFE_GENAPP, "GDdefdimscale", __FILE__, __LINE__);
	  HEreport("Did not find any hdfeos field in the grid \n");
	  status = -1;
 	  goto error;
	}
      else
...
-----------------------------------------------------------------------------*/
intn
GDsetdimscale(int32 gridID, char *fieldname, char *dimname,int32 dimsize, 
	      int32 numbertype, VOIDP data)
{
    intn            i;		/* Loop index */
    intn            status;	/* routine return status variable */
    ...
    int32           dims[8];	/* Dimension size array */
    int32           rank = 0;	/* Field rank */
    int32           slen;	/* String length array */
    int32           idOffset = GDIDOFFSET;	/* Grid ID offset */
    ...
    float64         pixsizeX;	/* X pixel size*/
    float64         pixsizeY;	/* Y pixel size*/
    int32           size;
    /* future extension if needed
    int8           *data_buff8=NULL;
    uint8          *data_buffu8=NULL;
    int16          *data_buff16=NULL;
    uint16         *data_buffu16=NULL;
    int32          *data_buff32=NULL;
    uint32         *data_buffu32=NULL;
    float32        *data_bufff32=NULL;
    */
    float64        *data_bufff64=NULL;
    void           *data_buff=NULL;
    int32           data_type=0;
    int32           count;
    char            name[2048];	/* Field Names */
    ...
    /*future extension if needed
    size = DFKNTsize(numbertype);

    switch (numbertype)
      {
      case DFNT_INT8:
	data_buff8 = (int8 *)calloc(dimsize, size);
	break;

      case DFNT_UINT8:
	data_buffu8 = (uint8 *)calloc(dimsize, size);
	break;

      case DFNT_INT16:
	data_buff16 = (int16 *)calloc(dimsize, size);
	break;

      case DFNT_UINT16:
	data_buffu16 = (uint16 *)calloc(dimsize, size);
	break;

      case DFNT_INT32:
	data_buff32 = (int32 *)calloc(dimsize, size);
	break;

      case DFNT_UINT32:
	data_buffu32 = (uint32 *)calloc(dimsize, size);
	break;

      case DFNT_FLOAT32:
      case DFNT_FLOAT:
	data_bufff32 = (float *)calloc(dimsize, size);
	break;

      case DFNT_FLOAT64:
	data_bufff64 = (float64 *)calloc(dimsize, size);
	break;
      }
    */

    data_bufff64 = (float64 *)calloc(dimsize, sizeof(float64));

    if(data_bufff64 == NULL)
    { 
	HEpush(DFE_NOSPACE,"GDsetdimscale", __FILE__, __LINE__);
	return(-1);
    }
     ...
     * and grid root Vgroup ID
     */
    status = GDchkgdid(gridID, "GDsetdimscale",
		       &fid, &sdInterfaceID, &gdVgrpID);
	...
	if (status != 0)
	{
	    HEpush(DFE_GENAPP, "GDsetdimscale", __FILE__, __LINE__);
	...
	if((data == NULL) || 
	   ...
	   (strcmp(dimname, "YDim") == 0))
	  {
	    ...
	    if (projcode == -1)
	      {
		HEpush(DFE_GENAPP, "GDsetdimscale", __FILE__, __LINE__);
		HEreport("Cannot get projection info for grid \n");
		status = -1;
		goto error;
	      }
		...
		if (status != 0)
		  {
		    HEpush(DFE_GENAPP, "GDsetdimscale", __FILE__, __LINE__);
		    HEreport("Cannot get grid default corner values. \n");
		    status = -1;
		    goto error;
		  }
	      }
		...
		/* -------------------------------------------------------- */
		upleftpt[0] = EHconvAng(upleftpt[0], HDFE_DMS_DEG);
		lowrightpt[0] = EHconvAng(lowrightpt[0], HDFE_DMS_DEG);
		upleftpt[1] = EHconvAng(upleftpt[1], HDFE_DMS_DEG);
		lowrightpt[1] = EHconvAng(lowrightpt[1], HDFE_DMS_DEG);
	      }
	...
	if(i == GDXGrid[gID].nSDS) /* did not find fieldname */
	  {
	    HEpush(DFE_GENAPP, "GDsetdimscale", __FILE__, __LINE__);
	    HEreport("Fieldname \"%s\" does not exist.\n", fieldname);
	    status = -1;
	...
	for (i = 0; i < rankSDS; i++)
	  {
	    /* Dimension name = Gridname:Dimname */
	    /* ---------------------------------- */
	    slen = strlen(dimname);
	    memcpy(utlbuf, dimname, slen);
	    utlbuf[slen] = 0;
	    strcat(utlbuf, ":");
	    strcat(utlbuf, gridname);
	    
	    dimid = SDgetdimid(sdid, i);
	    SDdiminfo(dimid, name, &count, &data_type, &dum);
	    if(strcmp(name,utlbuf) == 0 )
	      {
		break;
	      }
	    else
	...
	if (i == rankSDS) /* did not find appropriate dimname */
	  {
   	    HEpush(DFE_GENAPP, "GDsetdimscale", __FILE__, __LINE__);
	    HEreport("Dimname \"%s\" does not exist for field \"%s\".\n", 
		     dimname, fieldname);
	    status = -1;
	    goto error;
	  }
	...
	/* See if dimscale already set */
	if(data_type == 0)
	  {
	    status = SDsetdimscale(dimid, dimsize, numbertype, data_buff);
	    if (status != 0)
	      {
		HEpush(DFE_GENAPP, "GDsetdimscale", __FILE__, __LINE__);
		HEreport("Failed to set dimscale for \"%s\" in the field \"%s\".\n", 
			 dimname, fieldname);
		status = -1;
		goto error;
	      }
	  ...
	  {
	    HEpush(DFE_GENAPP, "GDsetdimscale", __FILE__, __LINE__);
	    HEreport("Dimscale for \"%s\" in the field \"%s\" has already been set.\n", 
		     dimname, fieldname);
	    status = 0;
	  }
    }
...
|  BEGIN_PROLOG                                                               |
|                                                                             |
|  FUNCTION: GDsetdimscaleF (Fortran wrapper)                                 |
|                                                                             |
|  DESCRIPTION: Defines a dimension scale for a dimension for fields of a grid|
|                                                                             |
|                                                                             |
...
-----------------------------------------------------------------------------*/
intn
GDsetdimscaleF(int32 gridID, char *fieldname, char *dimname,int32 dimsize, 
	      int32 numbertype, VOIDP data)
{
    ...
    if((strcmp(dimname, "XDim") == 0) || 
      ...
      {
	status =
	  GDsetdimscale(gridID, fieldname, dimname,dimsize, 
			numbertype, NULL);
      }
    else
      {
    ...
    if (status != 0)
      {
	HEpush(DFE_GENAPP, "GDsetdimscaleF", __FILE__, __LINE__);
	HEreport("Cannot set dimension scale. \n");
	status = -1;
      }
...
|  BEGIN_PROLOG                                                               |
|                                                                             |
|  FUNCTION: GDdefdimscaleF (Fortran wrapper)                                 |
|                                                                             |
|  DESCRIPTION: Defines a dimension scale for a dimension for all  fields of  |
|               a grid                                                        |
|                                                                             |
|                                                                             |
|  Return Value    Type     Units     Description                             |
...
-----------------------------------------------------------------------------*/
intn
GDdefdimscaleF(int32 gridID, char *dimname,int32 dimsize, 
	      int32 numbertype, VOIDP data)
{
    intn            status = -1;	/* routine return status variable */
    ...
    if((strcmp(dimname, "XDim") == 0) || 
      ...
      {
	status =
	  GDdefdimscale(gridID, dimname, dimsize, 
			numbertype, NULL);
      }
    else
      {
    ...
    if (status != 0)
      {
	HEpush(DFE_GENAPP, "GDdefdimscaleF", __FILE__, __LINE__);
	HEreport("Cannot set dimension scale for a dimension in all grid field. \n");
	status = -1;
      }
...
|  BEGIN_PROLOG                                                               |
|                                                                             |
|  FUNCTION: GDdefdimstrs                                                     |
|                                                                             |
|  DESCRIPTION: Defines a dimension scale label, unit and format for          |
...
|  gridID         int32               grid structure ID                       |
|  dimname        char                dim name                                |
|  label          char                Lable string for dimension (Gris X Axix)|
|  unit           char                unit for dimension (such as meter)      |
|  format         char                format in which numeric values to be    |
|                                     printed (such as F7.2)                  |
|                                                                             |
|                                                                             |
|  OUTPUTS:                                                                   |
|             None                                                            |
...
-----------------------------------------------------------------------------*/
intn
GDdefdimstrs(int32 gridID, char *dimname, char *label, char *unit, char *format)
{
    intn            i;		/* Loop index */
    intn            status;	/* routine return status variable */
    ...
    int32           sdInterfaceID;	/* HDF SDS interface ID */
    int32           sdid;	/* SDS object ID */
    int32           swVgrpID;	/* Grid root Vgroup ID */
    int32           dims[8];	/* Dimension size array */
    int32           idOffset = GDIDOFFSET;	/* Grid ID offset */
    int32           gID;	        /* GridID - offset */
    int32           rank = 0;	        /* Field rank */
    int32           rankSDS=0;	        /* Rank of SDS */
    int32           dum;	        /* Dummy variable */
    char            fieldname[2048];	/* Field Names */
    int32           data_type=0;
    int32           foundField=0;
     ...
     * and grid root Vgroup ID
     */
    status = GDchkgdid(gridID, "GDdefdimstrs",
      ...
      if(GDXGrid[gID].nSDS > 0)
	{
	  for (i = 0; i < GDXGrid[gID].nSDS; i++)
	      ...
	      if (GDXGrid[gID].sdsID[i] != 0)
		{
		  ...
		  if (status != 0)
		    {
		      status = -1;
		      HEpush(DFE_GENAPP, "GDdefdimstrs", __FILE__, __LINE__);
		      HEreport("Field: \"%s\" not found.\n", fieldname);
		      goto error;
		    }
		  ...
		  if (Dimindex == -1)
		    ...
		    {
		      foundField = 1;
		      GDsetdimstrs(gridID, fieldname, dimname, label, unit, format);
		      if ( status != 0 )
			{
			  status = -1;
			  HEpush(DFE_GENAPP, "GDdefdimstrs", __FILE__, __LINE__);
			  HEreport("Failed to set dimstrs for \"%s\" in the field \"%s\".\n", 
				   dimname, fieldname);
			  goto error;
			}
		    }
      ...
      if(foundField == 0) /* did not find any fields */
	{
	  HEpush(DFE_GENAPP, "GDdefdimstrs", __FILE__, __LINE__);
	  HEreport("Did not find any hdfeos field in the grid with the dimension specified\n");
	  status = -1;
	  goto error;
	}
      else
...
|  BEGIN_PROLOG                                                               |
|                                                                             |
|  FUNCTION: GDsetdimstrs                                                     |
|                                                                             |
|  DESCRIPTION: Defines a dimension scale label, unit and format              |
|                                                                             |
|                                                                             |
|  Return Value    Type     Units     Description                             |
|  ============   ======  =========   =====================================   |
...
-----------------------------------------------------------------------------*/
intn
GDsetdimstrs(int32 gridID, char *fieldname, 
	     char *dimname, char *label, char *unit, char *format)
{
    intn            i;		/* Loop index */
    intn            status;	/* routine return status variable */
     ...
     * and grid root Vgroup ID
     */
    status = GDchkgdid(gridID, "GDsetdimstrs",
		       &fid, &sdInterfaceID, &gdVgrpID);
	...
	if (status != 0)
	{
	    HEpush(DFE_GENAPP, "GDsetdimstrs", __FILE__, __LINE__);
	    HEreport("Fieldname \"%s\" does not exist.\n", fieldname);
	    status = -1;
	    goto error;
	}
	...
	if(data_type != 0)
	  {
	    status = SDsetdimstrs(dimid, label, unit, format);
	    if (status != 0)
	      {
		HEpush(DFE_GENAPP, "GDsetdimstrs", __FILE__, __LINE__);
		HEreport("Failed to set dimstrs for \"%s\" in the field \"%s\".\n", 
	  ...
	  {
	    HEpush(DFE_GENAPP, "GDsetdimstrs", __FILE__, __LINE__);
	    HEreport("Dimscale for \"%s\" in the field \"%s\" has not been been set yet.\n", 
		     dimname, fieldname);
	    status = -1;
	    goto error;
	  }
...
|  Return Value    Type     Units     Description                             |
|  ============   ======  =========   =====================================   |
|  status         intn                return databuffsize  SUCCEED, (-1) FAIL |
|                                                                             |
|  INPUTS:                                                                    |
|  gridID         int32               grid structure ID                       |
|  fieldname      char                field name                              |
...
|  OUTPUTS:                                                                   |
|  dimsize        int32               Dimemsion size value                    |
|  data_type      int32               dtata type of dimension scale           |
|  data           void                data buffer for read                    |
|                                                                             |
|  NOTES:                                                                     |
|                                                                             |
|                                                                             |
...
-----------------------------------------------------------------------------*/
intn
GDgetdimscale(int32 gridID, char *fieldname, char *dimname, int32 *dimsize, 
	      int32 *data_type, VOIDP data)
{
     ...
     * and grid root Vgroup ID
     */
    status = GDchkgdid(gridID, "GDgetdimscale",
		       &fid, &sdInterfaceID, &gdVgrpID);
	...
	if (status != 0)
	{
	    HEpush(DFE_GENAPP, "GDgetdimscale", __FILE__, __LINE__);
	    HEreport("Fieldname \"%s\" does not exist.\n", fieldname);
	    status = -1;
	    return ( status );
	}
	...
	/* Found the dimname; Use its dimid to get Dimension Scale*/
	/* if data == NULLjust return size of dimension and type for 
	   appropriate data buffer setting */
	if( data == NULL)
	  {
	    if(*data_type != 0)	/* See if dimscale is set */
	      {
		*dimsize = count;
		return ( count * DFKNTsize(*data_type));
	      }
	    else
	      {
		HEpush(DFE_GENAPP, "GDgetdimscale", __FILE__, __LINE__);
		HEreport("Dimscale for \"%s\" in the field \"%s\" has not been set.\n", 
			 dimname, fieldname);
		status = -1;
		return ( status );
	      }
	  ...
	  {
	    /* See if dimscale is set */
	    if(*data_type != 0)
	      {
		status = SDgetdimscale(dimid, data);
		if (status != 0)
		  {
		    HEpush(DFE_GENAPP, "GDgetdimscale", __FILE__, __LINE__);
		    HEreport("Failed to get dimscale for \"%s\" in the field \"%s\".\n", 
			     dimname, fieldname);
		    status = -1;
		    return ( status );
		  }
...
|  BEGIN_PROLOG                                                               |
|                                                                             |
|  FUNCTION: GDgetdimstrs                                                     |
|                                                                             |
|  DESCRIPTION: Get dimension strs for a dimension of a field in a grid       |
|                                                                             |
|                                                                             |
|  Return Value    Type     Units     Description                             |
|  ============   ======  =========   =====================================   |
|  status         intn                return 0  SUCCEED, (-1) FAIL            |
|                                                                             |
|  INPUTS:                                                                    |
|  gridID         int32               grid structure ID                       |
|  fieldname      char                field name                              |
|  dimname        char                dim name                                |
|  len            intn                Maximum string length it is safe to     |
|                                     return                                  |
|                                                                             |
|                                                                             |
|  OUTPUTS:                                                                   |
...
-----------------------------------------------------------------------------*/
intn
GDgetdimstrs(int32 gridID, char *fieldname, 
	     char *dimname, char *label, char *unit, char *format, intn len)
{
    intn            i;		/* Loop index */
    intn            status;	/* routine return status variable */
     ...
     * and grid root Vgroup ID
     */
    status = GDchkgdid(gridID, "GDgetdimstrs",
		       &fid, &sdInterfaceID, &gdVgrpID);
	...
	if (status != 0)
	{
	    HEpush(DFE_GENAPP, "GDgetdimstrs", __FILE__, __LINE__);
	    HEreport("Fieldname \"%s\" does not exist.\n", fieldname);
	    status = -1;
	    return ( status );
	}
	...
	if(i == GDXGrid[gID].nSDS) /* did not find fieldname */
	  {
	    HEpush(DFE_GENAPP, "GDgetdimsctrs", __FILE__, __LINE__);
	    HEreport("Fieldname \"%s\" does not exist.\n", fieldname);
	    status = -1;
	    return ( status );
	  }
	...
	if(data_type != 0)
	  {
	    status = SDgetdimstrs(dimid, label, unit, format, len);
	    if (status != 0)
	      {
		HEpush(DFE_GENAPP, "GDgetdimstrs", __FILE__, __LINE__);
		HEreport("Failed to get dimscale strs for \"%s\" in the field \"%s\".\n", 
			 dimname, fieldname);
		status = -1;
		return ( status );
	      }
...

{
	...
	if (status == 0)
	{
		...
		/* For SZIP compression currently one cannot do partial 
		   compression. So offset and count cannot be anything 
		   other than the min and max of these numbers respectively. 
		   So if SDwritedata fails it may be because of this. Lets 
		   inform the user if this is the case */
		if ( status != 0 )
		  {
		    status1 = GDcompinfo(gridID, fieldname, &compcode, NULL);
		    if (compcode == HDFE_COMP_SZIP)
		      {
			SZ_unsuppoted_flag = 0;
			if (rankFld == rankSDS)
			  {
			    for (i = 0; i < rankSDS; i++)
			      {
				if(offset[i] != 0 || count[i] != dims[i]) 
				  {
				    SZ_unsuppoted_flag = 1;
				    break;
				  }
			      }
			  }
			  ...
			  {
			    /*
			     * If field really 2-dim merged in 3-dim 
			     * field then set 0th field count has been 
			     * set to 1 and the next two to the user
			     * values.
			     */
			    for (i = 0; i < rankFld; i++)
			      {
				if(offset[i] != 0 || count[i + 1] != dims[i])
				  {
				    SZ_unsuppoted_flag = 1;
				    break;
				  }
			...
			if(SZ_unsuppoted_flag == 1)
			  {
			    HEpush(DFE_GENAPP, "GDwrrdfield", __FILE__, __LINE__);
			    HEreport("Possibly unsupported start and/or edge values for SZIP compression.Partial compression OF SDS is not allowed.\n");
			  }
		      }
		  }
	    }
...
|  Return Value    Type     Units     Description                             |
|  ============   ======  =========   =====================================   |
|  nattr          int32               Number of attributes in grid  struct    |
|                                                                             |
|  INPUTS:                                                                    |
|  grid ID        int32               grid structure ID                       |
|                                                                             |
|  OUTPUTS:                                                                   |
|  attrnames      char                Attribute names in grid  struct         |
...
|  Return Value    Type     Units     Description                             |
|  ============   ======  =========   =====================================   |
|  nFld           int32               Number of fields in grid                |
|                                                                             |
|  INPUTS:                                                                    |
|  gridID         int32               grid structure ID                       |
|                                                                             |
...
GDsetfillvalue(int32 gridID, char *fieldname, VOIDP fillval)
{
    ...
    char            name[84];	/* Fill value "attribute" name */
...

{
	...
	else
	{
	    ...
	    if(nflds < 1)
	      {
		HEpush(DFE_GENAPP,"GDdetach", __FILE__, __LINE__);
		return(-1);
	      }
	...
	for (i = 0; i < nflds; i++)
	{
		...
		for (j = i + 1, cmbfldcnt = 0; j < nflds; j++)
		{
		     ...
		     * 2nd for rank=2 array), grid ID, and numbertype
...
|  Sep 96   Raj Gejjaga   Fixed  bugs in Polar Stereographic and Goode        | 
|                         Homolosine default calculations.                    |
...
/* The following function is not used in hdfeos. commenting it out
   04/23/2010
*/
...
|  Jun 00   Abe Taaheri   Added support for EASE grid                         |
|  Feb 08   Abe Taaher    Created Fortran Wrapper, and made it an API         |
...

{
    ...
    if (projcode != GCTP_GEO)
    {
	/* Put the followings if State Plane conversion requied in future.
	   Some modification may be necessary, such as adding function
	   getEastAndNorthFiles from HEG. 
	   eastFile=../gctp/database/nad27sp
	   northFile=../gctp/database/nad83sp
	*/
	eastFile = (char *) malloc(300*sizeof(char));
	northFile = (char *) malloc(300*sizeof(char));

        GDgetEastAndNorthFiles(eastFile,northFile); 
	...
	float64 longitude[], float64 latitude[], int32 pixcen, int32 pixcnr)
{
	...
	kz_cea = cosphi1/(sqrt(1.0 - (eccen_sq*sinphi1*sinphi1)));
    ...
    else /* fail for all other projections */
      {
	    ...
	    HEreport("Error: GDrs2ll can be called only for GCTP_BCEA projection.");
	    return status;
      }
...
GDdefboxregion(int32 gridID, float64 cornerlon[], float64 cornerlat[])
{
	...
	/* in linux we get one more row and column when corner lat/lon
           are on mesh points(small floating point difference with unix
	   platforms results in round up to 1 mesh line higher than expected)
	   than unix platforms. To avoid this we modify enterd lower lat and
	   right lon slightly.
	*/
	/* ------------------------------------- */
	...
	latitude[1] = cornerlat[1]+ 1.e-10;

	longitude[2] = cornerlon[1]- 1.e-10;
	latitude[2] = cornerlat[0];

	longitude[3] = cornerlon[1]- 1.e-10;
	latitude[3] = cornerlat[1]+ 1.e-10;
               ...
               tiledims, int32 compcode, intn* compparm)
{
	...
	status = HCget_config_info(COMP_CODE_SZIP, &can_encode);
		...
		if(can_encode == (COMP_DECODER_ENABLED|COMP_ENCODER_ENABLED))
		  {
		    ...
		    c_info.szip.pixels_per_block = compparm[0];
		    c_info.szip.options_mask = compparm[1];
		  }
		else
		  {
		    HEpush(DFE_GENAPP, "GDsettilecomp", __FILE__, __LINE__);
		...
		if(can_encode == (COMP_DECODER_ENABLED|COMP_ENCODER_ENABLED))
		  {
		    /* pixels_per_block is even number from 2-32 */
		    chunkDef.comp.cinfo.szip.pixels_per_block = c_info.szip.pixels_per_block;
		    chunkDef.comp.cinfo.szip.options_mask = c_info.szip.options_mask;
		  }
	      }
#endif
...
|  ======   ============  =================================================   |
|  Oct 02   Abe Taaheri   Added support for EASE grid                         |
|  Dec 05   Tom Roegner   Added defined(CYGWIN)                               |
|                                                                             |
|  END_PROLOG                                                                 |
-----------------------------------------------------------------------------*/
...
|  BEGIN_PROLOG                                                               |
|                                                                             |
|  FUNCTION: GDgetEastAndNorthFiles                                           |
|                                                                             |
...
FCALLSCFUN15(INT,GDll2ij, GDLL2IJ, gdll2ij, INT32, INT32, DOUBLEV,
	     INT32, INT32, INT32, DOUBLEV, DOUBLEV,
	     INT32, DOUBLEV, DOUBLEV, INT32V, INT32V, DOUBLEV, DOUBLEV)

FCALLSCFUN15(INT,GDij2ll, GDIJ2LL, gdij2ll, INT32, INT32, DOUBLEV,
	     INT32, INT32, INT32, DOUBLEV, DOUBLEV,
	     INT32, INT32V, INT32V, DOUBLEV, DOUBLEV, INT32, INT32)

FCALLSCFUN5(INT, GDdefdimscaleF, GDDEFDIMSCALE, gddefdimscale, INT32, 
	    STRING, INT32, INT32, PVOID)
FCALLSCFUN6(INT, GDsetdimscaleF, GDSETDIMSCALE, gdsetdimscale, INT32, 
	    STRING, STRING, INT32, INT32, PVOID)
FCALLSCFUN6(INT, GDgetdimscale, GDGETDIMSCALE, gdgetdimscale, INT32, 
	    STRING, STRING, PINT32, PINT32, PVOID)
FCALLSCFUN5(INT, GDdefdimstrs, GDDEFDIMSTRS, gddefdimstrs, INT32, 
	    STRING, STRING, STRING, STRING)
FCALLSCFUN6(INT, GDsetdimstrs, GDSETDIMSTRS, gdsetdimstrs, INT32, 
	    STRING, STRING, STRING, STRING, STRING)
FCALLSCFUN7(INT, GDgetdimstrs, GDGETDIMSTRS, gdgetdimstrs, INT32, 
	    STRING, STRING, PSTRING, PSTRING, PSTRING,INT)
